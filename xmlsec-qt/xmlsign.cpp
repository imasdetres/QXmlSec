...
bool QXmlSign::sign()
{
  ...
  switch (sslCertificateType)
  {
    case XmlsecCertificate:
      ...
      dsigCtx->signKey = xmlSecCryptoAppKeyLoad(sslCertificate.xmlsec.filepath().toUtf8().constData(), sslCertificate.xmlsec.xmlsecFormat(), sslCertificate.xmlsec.password().toUtf8().constData(), NULL, NULL);
      break ;
    case QtCertificate:
